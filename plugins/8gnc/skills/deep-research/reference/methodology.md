...
Before any searches, retrieve today's date from the current runtime or a local shell. Use the returned year for date-filtered queries and recency checks. Do not assume a year from model memory.

**Step 1: Launch searches concurrently when the runtime supports it**

Choose one search approach per research session and follow that tool's actual schema:

- Use the current surface's built-in web search and page retrieval by default.
- Use an available semantic-search MCP only when its schema is known in the current runtime.
- Use `search-cli` only on a local execution surface, after the user has approved installation and configured their own provider keys.

Do not mix parameter styles across tools. Open the most promising results; search snippets alone are not evidence.

Example query lanes:

- `quantum computing [CURRENT_YEAR] state of the art`
- `quantum computing limitations challenges`
- `quantum computing commercial applications [CURRENT_YEAR]`
- `quantum computing vs classical comparison`
- `quantum error correction research`, restricted to primary research domains when supported

**Step 2: Run independent deep-dive lanes when available and authorized**

Use the runtime's parallel-agent mechanism only when it exists and the task permits it. Otherwise run the lanes sequentially. Useful lanes include:

- academic paper analysis;
- technical specification review;
- repository analysis;
- industry and market evidence;
- counter-evidence and competing hypotheses.

Require every lane to return structured evidence rather than unsupported prose:

```json
{"claim": "specific claim text", "evidence_quote": "short supporting excerpt", "source_url": "https://...", "source_title": "...", "confidence": 0.85}
```

Respect source quotation limits and verify each source directly before synthesis.
...
**Techniques:**
- Use the available web-search tool for current information
- Use search-cli for multi-provider aggregated search (if installed)
- Use the available page-retrieval tool for deep dives into specific sources
- Use an available semantic-search tool for exploratory queries
- Use Grep/Read for local documentation
- Execute code for computational analysis (when needed)
- Use parallel retrieval agents only when the runtime supports them and the task permits them
...
Use the runtime's parallel-agent mechanism, when available and permitted, for:
