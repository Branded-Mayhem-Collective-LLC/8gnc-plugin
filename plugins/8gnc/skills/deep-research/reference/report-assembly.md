...
Respect the current surface's output limit; do not assume a fixed token ceiling.
...
```bash
# Create a user-approved workspace folder
mkdir -p ./research-output/[folder_name]
...
- Location: a user-approved workspace path such as `./research-output/[TopicName]_Research_[YYYYMMDD]/`
...
Do not create a second hidden tracking copy. Keep provenance and continuation state beside the user-approved report.
