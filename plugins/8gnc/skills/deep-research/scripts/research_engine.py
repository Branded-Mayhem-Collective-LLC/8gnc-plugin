#!/usr/bin/env python3
"""
Provider-neutral deep research engine
...
class ResearchEngine:
    ...
    def __init__(self, mode: ResearchMode = ResearchMode.STANDARD):
        ...
        self.output_dir = Path.cwd() / "research-output"
...
## Execute:
1. Use the available web-search tool with iterative query refinement (minimum 10 searches)
2. Open and retrieve the 5-10 most promising sources
...
## Tools to Use:
- Web search: For current information and broad coverage
- Page retrieval: For detailed extraction from specific URLs
- Grep/Read: For local documentation if relevant
- Parallel agents: Use only when the runtime supports them and the task permits them
...
"""
    ...
    def execute_phase(self, phase: ResearchPhase) -> Dict[str, Any]:
        ...
        # In real usage, the model executes these instructions.
        # This returns a structured result for the model to populate.
        result = {
            'phase': phase.value,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Provider-neutral deep research engine",
...
Examples:
  ...
  python research_engine.py -q "longevity biotech funding trends" -m ultradeep
    ...
    print("\nNow execute each phase using the displayed instructions.")
